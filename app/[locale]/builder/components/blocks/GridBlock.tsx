...
function GridColumn({
  ...
  const selectedColumnIndex = useCanvasStore(
    (state) => state.selectedColumnIndex,
  );
  ...
  return (
    <div
      ...
      className={cn(
        ...
        !isPreviewMode &&
          (isColumnSelected || isParentSelected) &&
...
export function GridBlock({ block, isPreviewMode = false }: GridBlockProps) {
  ...
  const initialRatios =
    (block.data.ratios as number[]) || Array(columns).fill(1);
